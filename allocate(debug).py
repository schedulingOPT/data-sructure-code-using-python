...
def EnQueue(e,pqu):		#结点e进队操作
    ...
    if e.i==n:				#到达叶子结点
        #System.out.printf("  一个解 "); disptmp(e); 
        if e.cost<bestc:				#比较更新最优解
            bestc=e.cost
            bestx=copy.deepcopy(e.x)
            print("  最优解bestc=",bestc,"bestx=",bestx) 
    else:heapq.heappush(pqu,e)			#非叶子结点进队

def disptmp(e):
    print("（i=%d,cost=%d,lb=%d,"%(e.i,e.cost,e.lb),end='')
    print("x=",e.x,end=',')
    print("used=",e.used,end=')')
